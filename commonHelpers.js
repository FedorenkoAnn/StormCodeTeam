import"./assets/header-f05e16ca.js";import{a as k,t as $}from"./assets/vendor-da6d1e6a.js";async function C(){return await k.get("https://books-backend.p.goit.global/books/top-books")}async function E(o){return await k.get(`https://books-backend.p.goit.global/books/category?category=${o}`)}async function _(o){return await k.get(`https://books-backend.p.goit.global/books/${o}`)}const y=document.querySelector(".modal-body"),l=document.querySelector(".backdrop"),T=document.querySelector(".modal-close-btn");let s,a=[];const v=localStorage.getItem("shoppingList");v&&(a=JSON.parse(v));T.addEventListener("click",g);document.addEventListener("click",async o=>{try{const t=o.target.closest(".hardcover-item")||o.target.closest(".js-card");if(t){const e=t.id,c=(await _(e)).data,r=x(c);q(),M(y,r),s&&s.parentNode&&s.parentNode.removeChild(s),s=document.createElement("button"),s.textContent="Add to Shopping List",s.classList.add("add-to-shopping-list-btn"),a.includes(e)&&(s.textContent="Remove from Shopping List"),s.addEventListener("click",()=>{j(e)}),y.appendChild(s)}}catch(t){console.error(t)}});l.addEventListener("click",o=>{o.target===l&&g()});document.addEventListener("keydown",o=>{o.key==="Escape"&&g()});function q(){l.style.display="block"}function g(){l.style.display="none"}function x(o){const{book_image:t,list_name:e,title:n,author:c,description:r}=o;return`<div class="image-container">
      <img src="${t}" alt="${e}" />
    </div>
