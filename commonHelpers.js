import{a as S,b as $}from"./assets/slider-3c2fffe8.js";import{a as k,t as E}from"./assets/vendor-da6d1e6a.js";async function C(){return await k.get("https://books-backend.p.goit.global/books/top-books")}async function T(o){return await k.get(`https://books-backend.p.goit.global/books/category?category=${o}`)}async function M(o){return await k.get(`https://books-backend.p.goit.global/books/${o}`)}const f=document.querySelector(".modal-body"),p=document.querySelector(".backdrop"),j=document.querySelector(".modal-close-btn"),v=document.querySelector(".modal-bottom-text");let a,c=[];const g="shoppingList",b=localStorage.getItem(g);b&&(c=JSON.parse(b));j.addEventListener("click",h);document.addEventListener("click",async o=>{try{const t=o.target.closest(".hardcover-item")||o.target.closest(".js-card");if(t){const e=t.id,s=(await M(e)).data,r={_id:s._id,book_image:s.book_image,title:s.title,list_name:s.list_name,description:s.description,author:s.author,buy_links:s.buy_links.slice(0,2)},l=x(r);q(),O(f,l),a&&a.parentNode&&a.parentNode.removeChild(a),a=document.createElement("button"),a.textContent=c.some(u=>u._id===e)?"Remove from Shopping List":"Add to Shopping List",a.classList.add("add-to-shopping-list-btn"),a.addEventListener("click",()=>{I(r)}),f.appendChild(a),localStorage.setItem(g,JSON.stringify(c))}}catch(t){console.error(t)}});p.addEventListener("click",o=>{o.target===p&&h()});document.addEventListener("keydown",o=>{o.key==="Escape"&&h()});function q(){p.style.display="block"}function h(){p.style.display="none"}function x(o){const{book_image:t,list_name:e,title:n,author:s,description:r}=o;return`<div class="image-container">
      <img src="${t}" alt="${e}" />
    </div>
